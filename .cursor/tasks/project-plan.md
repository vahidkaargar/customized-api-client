...
**Typing / OpenAPI:** The **tracked npm library** ships **generic JSON:API primitives only** — **no** in-repo codegen, **`src/generated/openapi.ts`**, or **`paths`** re-exports. Consumer typing is **`README.md`** (“Typing your API”). Hard policy lives in **[`../rules/package-scope-no-openapi.mdc`](../rules/package-scope-no-openapi.mdc)**.

**Repository note:** Paths under **`.cursor/`** are often **gitignored** in clones; syncing agent docs for a team requires explicit git policy (exceptions in **`.gitignore`**).
...
| **OpenAPI / typings** | **Published package:** **no** OpenAPI codegen, **no** committed **`openapi/`** dir, **no** **`src/generated/openapi.ts`**, **no** re-export **`paths`** / **`operations`** / **`components`**. Consumers run **`openapi-typescript`** (or equivalent) elsewhere; document in **`README.md`**. Guard tests: **`test/unit/package-no-openapi.test.ts`**, **`test/postbuild/dist-no-openapi.test.ts`**. **Local Cursor mirror:** **`.cursor/api-documentations/openapi/v1.yaml`** (optional endpoint reference—not shipped on npm). |
...
/
  ...
  vitest.config.ts          # main suite; excludes test/postbuild/**
  vitest.postbuild.config.ts
  README.md
  src/
    index.ts
  ...
  test/
    unit/
    integration/            # MSW
    postbuild/             # asserts on dist/*.d.ts (run after npm run build)
```
...
## 10. Typing APIs and OpenAPI reference (consumers)

- **Consumers** who want strict HTTP typings run **`openapi-typescript`** (or another generator) against **their backend** spec or **`@myorg/api-types`**; call **`README.md`** § “Typing your API”.
- **This repository** must **remain decoupled:** no **`src/generated/openapi.ts`**, no **`openapi-typescript`**/`openapi:generate` in **`package.json`**, **no CI** job regenerating client types **into src**, **no `export type { paths`** from **`src/index.ts`**.
- **Regression guards:**
  - **`test/unit/package-no-openapi.test.ts`** — rejects **`generated/openapi`** in **`src/index.ts`**, forbids **`openapi/v1.yaml`**, **`npm pack --dry-run`** must not mention OpenAPI bundle paths.
  - **`test/postbuild/dist-no-openapi.test.ts`** — after **`npm run build`**, asserts **`dist/index.d.ts`** has **no OpenAPI façade** interfaces (run via **`vitest.postbuild.config.ts`** — same ordering as **[`.github/workflows/ci.yml`](../../.github/workflows/ci.yml)**).

Local optional mirror for agents (**not** authoritative): **`.cursor/api-documentations/openapi/v1.yaml`**.

---

## 11. Tests — exhaustive inventory (all required)

**Coverage / thresholds:** match **[`vitest.config.ts`](../../vitest.config.ts)** at **`src/**`** — currently **100%** statements/branches/functions/lines on included files **excluding only** **`src/types/jsonapi.ts`** and **`src/types/results.ts`** (declarative type modules). **Every exported public symbol** in measured files ≥ **1** test hit.

### Unit (`test/unit/`)

| File | Must assert |
|------|-------------|
| `export-surface.test.ts` | Runtime exports parity; rejects **`paths`/`operations`/`components`** leakage |
| `package-no-openapi.test.ts` | **`src/index.ts`** + **`npm pack`** contain **no** OpenAPI bundle/codegen artifact paths |
...
### Post-build (`test/postbuild/`)

| File | Must assert |
|------|-------------|
| `dist-no-openapi.test.ts` | Built **`dist/index.d.ts`** omits **`paths`** / generated OpenAPI surface |
...
Install + `createApiClient`; **Mode B `baseURL`** paragraph; idempotency (same key + body); **`ifMatch`**; pagination + follow next; **`Accept-Language`**; guards + validation pointers; security; retry defaults/disable; **“Typing your API”** (consumer-side OpenAPI / generics); **Development** section lists **`typecheck` → `lint` → `test:coverage` → `build` → post-build Vitest**.
...
| [`phase-09-transform-response-keys.md`](phase-09-transform-response-keys.md) | **`transformResponseKeys`** (historical OpenAPI-in-package phase removed) |
...
**Legacy step map:** scaffold → types/parsers → headers → redact → retry → axios → client → helpers → transform → integration → ship.
...
- [ ] **§11** every test file exists; coverage **matches** **[`vitest.config.ts`](../../vitest.config.ts)** thresholds on measured **`src/**`**.
...
- No backend changes.
- No OpenAPI codegen, committed **`openapi/`** spec, or **`src/generated/openapi.ts`** inside **this** package (see §A, §10, **[`../rules/package-scope-no-openapi.mdc`](../rules/package-scope-no-openapi.mdc)**).
...
**End:** An agent completing **§14 phases** with **§11 tests** and **§15 checklist** must not omit idempotency, concurrency, statuses, retries, dual error API, Mode **B** defaults, **consumer-typing** policy (no in-package OpenAPI), pagination legacy URLs, or DX helpers.
