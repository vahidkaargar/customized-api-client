...
For the **same command sequence as CI** (`typecheck` → lint → coverage → **`build`** → **post-build Vitest**), see [`release-and-ci-parity.md`](release-and-ci-parity.md).
...
Bracket params per [project-plan §9 / helpers](../tasks/project-plan.md). Wire format stays **`snake_case`** in attributes; query keys follow **JSON:API**, typically matching the backend contract (OpenAPI) when you have a spec **as reference**—never via codegen shipped from **this package**.
